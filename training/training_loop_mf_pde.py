...
# Increase NCCL timeout to prevent hangs in distributed training
if 'TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC' not in os.environ:
    os.environ['TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC'] = '3600'  # 1 hour timeout
...
def save_image_grid(img, fname, drange, grid_size):
    ...
    if C == 1:
        ...
        # For 2-channel data, save first channel as grayscale
        PIL.Image.fromarray(img[:, :, 0], 'L').save(fname)
    else:
        # For other channel counts, save average of all channels as grayscale
        avg_img = img.mean(axis=2).astype(np.uint8)
        PIL.Image.fromarray(avg_img, 'L').save(fname)
...
def manual_grad_sync(model):
    """Sync gradients across ranks (DDP should handle this, but for custom loss handling)."""
    for param in model.parameters():
        if param.grad is not None:
            # Average gradients across all ranks
            tdist.all_reduce(param.grad, op=tdist.ReduceOp.SUM)
            param.grad.div_(tdist.get_world_size())
...
def training_loop(
    ...
    ddp = torch.nn.parallel.DistributedDataParallel(net, device_ids=[device], broadcast_buffers=False, find_unused_parameters=False, gradient_as_bucket_view=True)   # Use DDP for gradient synchronization
    ...
    while True:
        ...
        # Ensure all ranks have completed backward pass before stepping
        tdist.barrier()
